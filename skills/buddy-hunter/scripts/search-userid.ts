...
// --- Load checkpoint ---
type Checkpoint = { prefixIdx: number; numIdx: number; best: ReturnType<typeof rollFull> | null; found: number }
...
function saveCheckpoint(prefixIdx: number, numIdx: number, best: ReturnType<typeof rollFull> | null, found: number) {
  if (!CHECKPOINT_FILE) return
  Bun.write(CHECKPOINT_FILE, JSON.stringify({ prefixIdx, numIdx, best, found }))
}

// --- Search ---
console.log(`Searching: species=${TARGET_SPECIES||'any'} rarity=${TARGET_RARITY||'any'} shiny=${WANT_SHINY} stat=${WANT_STAT||'any'}=100 min-total=${MIN_TOTAL} stop-after=${STOP_AFTER}`)
...
for (let pi = checkpoint.prefixIdx; pi < PREFIXES.length && !stopped; pi++) {
  ...
  for (let i = startNum; i < LIMIT; i++) {
    ...
    if (!best || r.total > best.total) {
      best = r
      foundCount++
      console.log(`[${foundCount}] Found: total=${best.total} uid="${best.uid}" ${best.rarity}${best.shiny?' shiny':''} ${best.species} eye:${best.eye} hat:${best.hat}`)
      // Stop conditions: reached near-perfect OR found enough results
